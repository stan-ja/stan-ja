...
上のStanプログラムのコードでは、予測変数`x[n]`と群レベルの係数`beta[jj[n]]``を使って、`x_beta_jj[n]`という平均を記述する（**?entry?**）ベクトルを組み立てていました。`x_beta_jj`が局所変数として宣言できるように、その周りに中括弧でブロックを定義しました。このベクトル化したコードは、より単純な、ベクトル化していない次のコードと等価です。
...
上にあるStanプログラムのコードではまた、結果と多変量正規分布に関係するところでベクトルの配列を組み立てています（**結果yはベクトルだが?**）。解いて微分する必要のある線形系の数を減らすことで、非常に顕著な高速化が可能になります。
...
多変量プロビットモデルは、見かけ上無関係な回帰の結果に階段関数を適用することにより一連の論理値の変数を生成します。

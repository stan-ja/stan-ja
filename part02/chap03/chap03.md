...
行列型は相関行列を表現するように制約をかけることができます。ある行列が相関行列だというのは，それが対称で正定値行列であることで，その要素が-1から1の間にあり，対格項は1であるということです。相関行列は正方行列なので，宣言に際して必要な数字は一次元だけです。例えば，
...
行列型変数は共分散行列のコレスキー因子で表現されるように制約をかけることができます。これは時に便利で，また共分散行列を直接表現するよりも効果的であることがあります。

共分散行列のコレスキー因子$L$は$M \times N$の下三角行列($m<n$であれば$L[m.n]=0)で，対角要素は正(L[k,k]>0)，かつ$M \ge N$です。もし`L`がコレスキー因子であれば，$\Sigma = LL^T$は共分散行列です。さらに，全ての共分散行列はコレスキー因子化できます。
...
二次元配列は配列の配列に過ぎずません。概念的にも，具体的な実装上もです。インデックスが配列に与えられたとき，そのインデックス番号に該当する数字が返されます。一つ以上のインデックスが渡されたときは，インデックスの操作が次々に繋がります。例えば，もし`a`が二次元配列だとすると，`a[m,n]`は`a[m][n]`の簡略化表記になります。ベクトル，行列，配列は互いに割り当てあうことができます。その次元数が同じであったとしても
...
は，15$\times$12の配列が$7\times 2$個あることになります。制約付きの型を配列に使うこともでき，次のように宣言すると，
...
配列の下位配列にアクセスするには，全体よりも少ないインデックスを与えてやることです。例えば，`y`を3行4列の二次元配列だとしましょう。ここで`y[3]`は長さ4の一次元配列を返します。これが意味するのは，`y[3][1]`が`y[3,1]`の代わりに使われたので，`y`の三番目の行の最初の列の値にアクセスしたことになるからです。`y[3,1]`の形式の方が，より好ましい形ではあります(この章の脚注3を見てください)。
...
サイズは合っていなければなりません。ここでは`x[1]`がJ\times$Kの配列で,`Y`も同じです。

部分的な配列の割り当ては，行列，ベクトル，行ベクトルの配列についても同様の動きをします。

#### 配列，ベクトル，行列の型の混在

配列，行ベクトル，列ベクトル，そして行列はStanでは相互に交換できません。ですから，これらの基本的な型のどの変数も，他の型のものに割り当てることができません。他の型が必要とされている引数で合ったとしても，です(引数として使うのは引数のルールに従います)。
...
ある変数に耐えられたサイズは，データ型の一部ではありません。例えば，変数を次のように宣言したとします。
...
関数が実行されるときにその引数として，制約が，いつもというわけではないですが，時々チェックされます。C++の標準ライブラリ関数については例外です。あらゆる確率関数，累積分布関数は，その関数が呼び出される実行時に引数が適切であるかどうかをチェックします。
...
変数を定義する全てのブロック(`transformed data`,`transformed parameters`,`generated quantities`)において，実数値は`NaN`として初期化され，整数値はアリエル最も小さい整数値として初期化されます(すなわち，大きな絶対値を持つ負の数)。
...
分の右辺にある式の型は，変数の型が宣言された時に配列可能でなければなりません。例えば，次のようにすることは可能です。

```
real sum = 0;
```

`0`は`int`型であるにも関わらず，`sum`は`real`型ですが，整数型のスカラーは実数型のスカラー変数に割り当て可能ですのでこの方法は許されます。他のどの場合でも，割り当てる右辺にある式の型は，変数の型と同じでなければなりません。
...
とすれば，行列変数`a`は`b+c`の和の半分が割り当てられます。`b`と`c`に求められているのは，式`b+c`が行列型担っているということだけです。例えば，`b`は行列型で，`c`が実数型であるということができます。なぜなら，スカラーに行列を加算すると行列型ができ，それにスカラーをかけることでもう一つの行列型になるからです。
...
式はStanプログラムにおける基本的な文法単位であり，値を示すものです。Stanプログラムに合う形で表現された全ての式は，(コンパイル時に)静的に定められている型を持っています。式の型が静的に定まらない場合，Stanコンパイラは問題の位置を報告します。

この章ではStanで使われる文法，型，様々な形式の用法を説明しています。

### 4.1 数字のリテラル

式の最も単純な形式は， 根本的な数字の値を指し示しているリテラルであることです。

#### 整数リテラル

整数のリテラルは`int`型を表しています。整数リテラルは区切りのない10進法で書かれます。整数リテラルは負の符号を一つことができます(`--1`はリテラル`-1`の否定と受け取られます)。

次のリストは，形式にあった整数リテラルです。

```
0,1,-1,256,-127098,24567898765
```

整数リテラルは整数値の限界の中にある値でなければなりません(セクション3.2を参照)。

整数リテラルは，小数点`(.)`を含んではいけません。ですので，式`1.`と`1.0`は実数型であって，`int`型の値が求められているところで使うことはできません。

#### 実数リテラル

ピリオドを持った数字や，科学的記数法は，連続的な数字型である`real`に割り当てられます。実数リテラルは分離記号であるピリオド`(.)`を含む10進法で書かれています。形式にあった実数リテラルの例は次のようなものです。

```
0.0,1.0,3.14,-217.9387,2.7e3,-2E-5
```

`e`や`E`の記述は，続く正あるいは負の数が10の指数をしめしており，それをかけたものになります。例えば，`2.7e3`は$2.7 \times 10^3$であり，`-2E-5`は$-2 \times 10^{-5}$を意味しています。

### 4.2 変数

変数自身は同じ型の変数による式で表現されたものです。Stanの変数はASCII文字の，基本的な小文字と大文字のローマ文字，数字，アンダースコア(`_`)文字からなります。変数は文字(a-zとA-Z)から始まらなければならず，二つのアンダースコア(`__`)で終わってはいけません。

ルールにあった変数の表現例は次のようなものです。

```
a,a3,a_3,Sigma,my_cpp_style_variable,myCamelCaseVariable
```

RやBUGSと違って，　Stanにおける変数表現はピリオドを含んでいてはいけません。

#### 予約名

Stanは内部で用いる多くの文字列を保管しており，これらを変数の名前として用いることはできません。変数を内的文字列にちなんで名付けようとすると，`stanc`変換が使われた予約語とモデルコードの位置と共に，エラーメッセージをつけて停止することになります。

##### モデル名

モデルの名前はモデルの中で変数名のように使うことはできません。これは普通モンヂにhんりません，というのも，`bin/stanc`のデフォルトはモデルを定式化した中身を持つファイルの名前に`_model`を追加して名前にするからです。例えば，モデルがファイル名`foo.stan`に書いてあるとすると，変数名に`foo_model`とすることは，デフォルトモデル名が`bin/stanc`にあるときは，適切ではありません。ユーザーが特定したモデル名，変数名はモデルと合致してはいけません。

##### ユーザー定義関数の名前

ユーザーが定義する関数の名前はモデルの中で変数として使うことはできません。

##### Stan言語による予約語

次のリストはStanプログラム言語において予約語とされているものです。これらの特徴の全てがStanに組み込まれているわけではないですが，今後使われることもあるので取り分けられているのです。

```
for, in, while, repeat, until, if, then, else, true, false
```

変数は型名にちなんでつけることはできませんので，次のどの名前であってもいけません。
```
...
いくつかの変数名は予約語です。というのも，これらはStanの組み込んだC++で使われているからです。該当するのは次のものです。
...
変数名は，定数を除いてすでに予約されている関数名とコンフリクトを起こします。ですから，変数名は`logit`とか`add`のような名前であってはいけませんが，`pi`や`e`は構いません。

変数名は分布の名前で，語尾に`_lpdf`,`_lpmf`,`_lccdf`,`_cdf`がついているような分布の名前，例えば`normal_lcdf_log`はコンフリクトを起こします。また推奨されない形式である`_log`,`_cdf_log`や`_ccdf_log`にも該当します。

これrの変数名を使うと，`stanc`変換器が停止するとともに，変数名と変数が使われてコンフリクトを起こした位置を報告します。

##### C++で予約されている名前

最後に，変数名およびモデル名も含みますが，それらはC++のキーワード共コンフリクトを越してはいけません。

```
alognas, alignof, and, and_eq, asm, auto, bitand, bitor, bool,
break, case, catch, char, cha16_t, char32_t, class, compl,
const, constexpr, const_cast, continue,decltype,default,delete,
do, double, dynamic_case, else, enmu, explicit,export,extern,
false, float, for, friend, goto , if, inline, int, ling, mutable,
...
変数の文字として使えるのは，Unicodeとおなじく，0-127の範囲にあるASCIIコードポイントです。

| 文字列 | ASCII(Unicode)のコードポイント |
|:-----------|------------:|
| a - z | 97 - 122 |
| A - Z | 65 - 90 |
| 0 - 9 | 48 - 57 |
...
もっとも表現力のある文字セットというわけではないですが，ASCIIはもっともポータブルで，エンコードやデコードにおいて不適切な文字にもっとも変化しにくいものです。

##### コメントはASCIIと互換可能なエンコーディングであれば結構です

コメントの中では，StanはASCIIと互換可能な文字コード，ASCIIそのものやUTF-8,Latin1であれば使えます。それが適切に表示されるかどうかはユーザーのシェルとエディタによって異なります。
...
一連の式を中括弧で括ることで，配列の式を表現できます。例えば，`{1,10,100}`は三つの要素1,10,100をもつ整数の配列を表しています。この書き方は特に，一行で小さな配列を表現するときに便利な書き方になります。

```
int a[3] = {1,10,100};
```

この値は式を含めることができるので，`{2*3,1+4}`のような書き方もできます。二次元配列を直接表現することもでき，次のような書き方ができます。

```
int b[2,3] = {{1,2,3},{4,5,6}};
```

このやり方では，`b[1]`は`{1,2,3}`で`b[2]`は`{4,5,6}`になります。

Stanでは空白をいつでも置くことができるので，上の式は次のように配置することで，もっとはっきり行と列の構造を表現し，二次元配列であることを示すことができます。
...
配列の式を作るにあたって，どのような式をかっこの中に入れても構いません。もっとも単純なケースでは，全ての要素が同じ型で，結果としてその型の要素を持つ配列になります。例えば，配列の要素はベクトルになることもでき，その場合はベクトルの配列ができます。
...
要素は`int`と`real`の型を混ぜた式でもyく，その場合は実数値の配列になります。
...
配列の式がどのように使われるかについては，計算された型がボトムアップでおこること，そしてStanの基本的なデータ型と割り当てルールによって制限されます。

#### 配列式は矩形だけ

不規則な配列式を定義しようとしても，すべてのStanのデータ型は矩形(あるいは正方形か，高次元に一般化されたもの)です。だから，次に示すようなネストされた配列式は，矩形でない配列を作ろうとするとエラーを引き起こします。

```
{{1,2,3},{4,5}} // コンパイル時にエラー：サイズミスマッチ
```

一見問題ないように見えます。というのも，二次元の整数配列(`int[ , ]`)は二つの一次元配列が整数の配列(`int[ ]`)になっているからです。しかしこれが不適切なのは，二次元配列は同じサイズでなければならないからです。もしその要素が配列式であれば，コンパイル時にそれが診断されます。もし一つ，あるいはいずれの式も変数であれば，実行時までエラーがキャッチされないかもしれません。
...
結果の型を参照する方法はないので，空の配列式(`{ }`)は許されていません。これは式の表現力を弱めるものではありません。というのも，ゼロ要素の配列を初期化するのは宣言で十分だからです。

```
int a[0];   // aは0要素の配列として十分に定義されます
```

#### 整数だけの配列式

配列式の要素は整数だけ，たとえば`{1,2,3}`のようなものでしたら，結果的に方は整数の配列，`int[]`になります。これが意味するのは，次のような表現は不適切であるということです。

```
real a[2] = { -3, 12};  // エラー；int[]はreal[]に割り当てできません
```
整数配列に実数値を割り当てることはできません。しかし，この問題は次のような表現を使うことで簡単に回避することができます。

```
real a[2] = {-3.0, -12.0};
```

今度は型は合っているし割り当ても認められています。

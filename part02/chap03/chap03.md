...
ユニットシンプレックスはベクトルとして実装され，他のベクトルと同じように割り当てられます。シンプレックス変数は，他の制約付き変数のように，要素の神父レックス値が妥当かどうかを評価されます。シンプレックスについては，浮動小数点の不正確さに起因するエラーを説明するために，静的に指定された閾値`\epsilon`までしか行われません。

...
すなわち、`c[idxs]`のようにインデックスを1番目の位置に置くと、上記で定義したのと全く同じように振る舞います。(* よくわかりません。The fact that the values are themselves arrays makes no difference *)結果は依然として`c[idxs][j] == c[idxs[j]]`によって定義されます。

...
`while`と`for`ループでは、これにより、ループの条件文へ制御が戻ります。
...
そのかわり、トップの条件文は排除する場合を見つけるようになっており、後者の、排除しない場合のコードと同じレベルのネスティングを必要としません。
...
`break`も`continue`もループの中に置かなくてはなりませんが、ループ内のネストした文の間に置くこともできます。たとえば、上の条件文の間や、ネストした文の間に置くことができます。

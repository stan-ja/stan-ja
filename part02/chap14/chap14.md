...
本章では、広く用いられている統計的なクラスタリングモデルの内2つ、ソフト$K$平均法と潜在ディリクレ配分法(LDA)に関して、Stanでの実装について説明します。
...
### ソフトK平均法
$K$平均クラスタリングとは、$D$次元のベクトルで表わされるデータをクラスタリングする方法です。
具体的に言うと、分類される項目が$N$個存在し、その各々はベクトル$y_{n} \in \mathbb{R}^{D}$で表わされます。
$K$平均法の「ソフト」版では、クラスタの割り当てが確率的になります。

#### 幾何的なハード$K$平均クラスタリング
$K$平均クラスタリングは以下のアルゴリズムの様に、通常幾何的な観点で記述されます(クラスタ数$K$とデータベクトル$y$は入力として仮定します)。

1. $1:N$の各$n$に対し、ベクトル$y_n$を$1:K$のとあるクラスタにランダムに割り当てます
2. 以下の処理を繰り返します
    (a) $1:K$の各クラスタ$k$に対し、そのクラスタに割り当てられたベクトルを平均してクラスタの重心$\mu_{k}$を計算します
    (b) $1:N$の$n$に対し、$y_n$から$\mu_{k}$への(ユークリッド)距離が最小となるクラスタ$k$に、$y_n$を再度割り当てます
    (c) どのベクトルもクラスタが変わらなくなったら、そのクラスタ割り当てを返します

このアルゴリズムは、停止することが保証されています。

#### ソフト$K$平均クラスタリング
ソフト$K$平均クラスタリングでは、クラスタの割り当ては複数のクラスタに渡る確率分布に基づき扱われます。
ユークリッド距離と共分散一定の多変量正規モデルの間の関係から、ソフト$K$平均法は、多変量正規分布の混合モデルとして(Stanのコードも)表現することができます。

この**`(訳注: full全？)`**生成モデルでは、$1:N$の各データ点$n$に、次のような対称で一様な確率でクラスタ$z_n \in 1:K$が割り当てられます。

![$$ z_n \sim \mathsf{Categorical}(\mathbf{1}/K), $$](fig/fig01.png)

ここで$\mathbf{1}$は$K$次元の単位ベクトルですので、$\mathbf{1}/K$は対称な$K$単体となります。
従ってこのモデルでは、クラスタメンバの硬判定結果から各データ点が抽出されると仮定されます。
「ソフト」さは、データ点を生成するクラスタの不確実性のみから発生します。

データ点自体は、割り当てられたクラスタ$z_n$から決定されるパラメータを持つ、多変量正規分布より生成されます。

![$$ y_n \sim \mathsf{Normal}(\mu_{z[n]},  \Sigma_{z[n]}) $$](fig/fig02.png)

本節でのサンプル実装では、共分散行列には全クラスタ$k$に共通して固定的に単位行列を仮定します。
...
$K$平均法に対する空間的な観点では、上式のかっこの内側の項が、まさにクラスタ平均$\mu_k$からデータ点$y_n$までのユークリッド距離(を半分にして符号を反転した値)になっていることに注意して下さい。

#### ソフト$K$平均法のStanでの実装
$K$平均クラスタリングを実装した、次のStanのプログラムを考えましょう。^[このモデルはStanのモデル例に関するリポジトリから入手可能です。<http://mc-stan.org/documentation>をご参照ください。]
```
data {
...
この事前分布は別の分布に変更してもよく、問題全般の尺度と位置に適合するような階層モデルにしてもよいでしょう。

パラメータは`mu`のみであり、`mu[k]`はクラスタ$k$の重心となります。
変換パラメータ`soft_z[n]`には、規格化されていないクラスタ割当確率の対数が含まれます。
ベクトル`soft_z[n]`は、Stanの外部か、もしくはモデルの生成量ブロックの中で、ソフトマックス関数を用いて規格化された単体に戻すことができます(34.11節もご参照ください)。
...
$K$平均法を多変量正規分布の視点でとらえる限り、共分散行列を単位行列からクラスタに共通する別の行列に変更しても、結局その共分散行列の逆行列で変換された空間で定義されるユークリッド距離で動作しているのと同じことになります。

なお**`(訳注: global包括的な？)`**空間的な類似性はありませんが、共分散行列がクラスタ毎に異なるソフト$K$平均法も一般的です。
この様な場合には、共分散行列に対して階層的な事前分布が用いられる事があります。

### クラスタリングにおけるベイズ推定のむずかしさ
クラスタリングモデルのフルベイズ推定はほぼ実行不可能であり、これはパラメータの識別可能性の欠如と事後分布の極度な多峰性の2つの問題によるものです。
20.2節には、ラベル交換に起因する識別不可能性について追加で議論が行われています。

#### 識別不可能性
クラスタの割り当ては、そもそも識別することができません。これは、クラスタの平均ベクトル`mu`を入れ替えたとしても、尤度の同じモデルが導れるためです。
例えば、`mu`の最初の2つのインデックスと各`soft_z[n]`の最初の2つのインデックスを入れ替えても、事前分布が同じであれば尤度も同じになります。**`(訳注: 原文priorですがposteriorと捉え、"・・・入れ替えても、尤度は(事後分布も)同じになります。"とした方が適当？)`**

このような識別可能性の欠如は、複数のマルコフ連鎖の間でクラスタのパラメータが比較できないことを意味しています。
ソフト$K$平均法におけるパラメータは一つですが、このパラメータが識別されないため、実際収束をモニタする際に問題が生じます。
...
しかしながらたとえインデックスの問題がなかったとしても、事後分布が非常に多峰的であること自体が問題です。

多峰性が高い場合、ベイズ推定は失敗します。これは、事後分布のあらゆる峰を適切な比率で調べる方法がなく、従って事後予測の推定に含まれる積分を評価する方法もなくなるためです。
...
多項混合モデルは「ナイーブベイズ」ともよばれます。これはこのモデルが、多項独立の仮定が明白に偽となる分類問題に適用されることが多いためです。

ナイーブベイズによる分類とクラスタリングは、多項的な構造をもつあらゆるデータに適用できます。
...
多項混合モデルでは、各ドキュメント$m \in 1:M$に対して、カテゴリカル分布に従い単一のカテゴリ$z_m \in 1:K$が生成されます。
...
Stanは不ぞろいな配列をサポートしていないため、このモデルのコード化には別のやり方(全単語を通じて各単語にインデックスを付与する方法)が用いられます。
このデータは次のように構成されており、一列目では単語の配列が複数配置され、二列目ではその各配列にとある文書が割り当てられています。
...
カテゴリが既知の文書が訓練データとして与えられた場合、ナイーブベイズモデルで単体上のパラメータを推定するStanのコードは、次のように書けます。^[このモデルはモデル例のリポジトリから入手可能です。<http://mc-stan.org/documentation>をご参照ください。]
```
data {
  // training data
...
```
トピックの識別子$z_m$がデータとして宣言され、潜在的なカテゴリの割り当てが尤度関数の一部として含まれていることに注意してください。
...
パラメータは事前分布を除いて先ほどと同じですが、今度の尤度は次のように文書について周辺化された確率として計算されます。
...
変数`gamma`を変換パラメータブロックで宣言して定義すると、Stanはそのサンプル値を保存します。
...
このモデルは本質的にはデータが欠測しているモデルであり、未知のランダムなカテゴリラベルが全て欠測していると仮定されます。
欠測データの補完に関する更なる情報については、 (Gelman et al., 2013; Gelman and Hill, 2007) をご参照ください。
またこのモデルは、ラベルの付与されていないデータがパラメータの推定に寄与を行うため、半教師あり学習の例にもなっています。
...
ラベルの付与されていない文書に関する`gamma`の定義を生成量ブロックに移行すると、この様な振る舞いを行う実装が可能になります。
この場合その変数はもはや対数確率には寄与しなくなるので、モデルパラメータの推定においても、もはや同時には影響しないことになります。

### 潜在ディリクレ配分法
潜在ディリクレ配分法(LDA)は、ナイーブベイズを一般化した、混合メンバーシップの多項クラスタリングモデル(Blei et al., 2003)です。
...
ここで事前分布の超パラメータ$\alpha$は一定であり、これは$K$個の正の値のベクトルとなります。
...
ここで$\beta$は一定であり、$V$個の正の値のベクトルとなります。

#### 離散パラメータの積分消去
Stanでは離散パラメータのサンプリングが(まだ)サポートされていませんが、他の混合モデルの場合と同じように離散パラメータを積分消去して連続パラメータのみの周辺分布にすることで、計算が可能になります。
...
#### 相関を持つトピックモデル
文書のトピック分布における相関を考慮するために、(Blei and Lafferty, 2007)ではLDAの変種が提案されており、そこでは文書毎のトピックに関する事前分布が、ディリクレ分布から多変量のロジスティク正規分布に置き換えられています。

この論文の筆者らは、超パラメータが一定の場合を扱っています。
なお論文では共分散の$L_1$正則化推定値が用いられていますが、これは事前分布に二重指数分布を設定した場合の最大事後確率推定値と等価です。
Stanでは最大事後確率推定は(まだ)サポートされていませんので、多変量のロジスティク正規分布の平均と共分散は、データとして規定する必要があります。

##### 超パラメータが一定の場合の相関を持つトピックモデル
前節におけるStanのモデルは、相関を持つトピックモデルを実装するように修正することができます。このためには、トピックの事前分布に関するデータの宣言において、ディリクレ分布における`alpha`を多変量のロジスティク正規分布における平均と共分散で置き換えます。
```
data {
...
```
そして、ディリクレ分布から単体となるパラメータ`theta`を抽出する代わりに、まず多変量正規分布からパラメータ `eta`を抽出し、次いでソフトマックスを用いてその値を単体に変換します。
```
parameters {
  simplex[V] phi[K];   // word dist for topic k
...
このためには、トピック分布に関する平均`mu`と共分散`Sigma`の宣言をデータブロックからパラメータブロックに移行し、それらの事前分布をモデル部で与える必要があります。
...
```
形状パラメータが$\alpha > 0$の`LkjCorr`分布は、相関行列(すなわち、対角項が1の対称な正定値行列)と同じ台を持ちます。
この密度は次のように定義されています。

![$$ \mathsf{LkjCorr}(\Omega \mid \alpha) \propto \det(\Omega)^{\alpha-1} $$](fig/fig17.png)

$\alpha = 2$というスケールでは、この分布は単位相関行列よりの弱情報事前分布となります。
したがって、多変量のロジスティク正規分布の共分散行列$\Sigma$にこの事前分布を適用すると、対角項への集中度がいくらか増すと共に、尺度は`sigma`の事前分布から決定される、という効果があわされることになります。

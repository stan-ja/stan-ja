...
$a$と$b$が対数スケールの確率なら、$\exp(a)+\exp(b)$は線形スケールでの両者の和になります。そして外側の対数で、結果を対数スケールに戻します。まとめると、log\_sum\_expは対数スケールで線形の加算を行なうということになります。Stanの組込み`log_sum_exp`関数を使う理由は、指数計算でのアンダーフローやオーバーフローを防ぐことができるということにあります。結果は以下のように計算されます。

...
３つ目のチューニングパラメタは，仮想粒子の質量行列です．Stanはこの対角行列となる質量行列や完全な質量行列を推定するために焼き捨てを通じて設定され，将来的にはユーザが指定した質量行列をサポートする予定です．巨大な対角質量行例を推定することは，未知の変数列 $\theta$ の元 $\theta_k$の縮尺を標準化することを意味します．一方完全な質量行列を推定すればることは縮尺に加え回転も考慮する事が出来ますが，それと引き換えに，leapfrogの各ステップの行列演算のためにより多くのメモリを必要とします．
...
Stanがベイズ推論を行うためには，事後確率関数においてこの比例を関係を記述することで，このベイズ則は事後確率分布が尤度関数と事前分布の積に比例するということと同値です．
...
もし事前分布が一様ならば，事後分布の最頻値を計算することは，パラメタの最尤推定（Maximum likelifood estimate = MLE）に対応します．また，事前分布が一様でなければ，この事後分布の最頻値の計算はしばしばMAP（Maximum a posterior）推定と呼ばれます．

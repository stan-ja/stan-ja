...
#### RStan
RStanはRにおけるStanのインターフェースです。R2WinBUGSやR2jagsでは、Rから外部のソフトウェアとしてWinBUGSやJAGSを呼び出していました。Stanではそうではなく、むしろRのメモリを通じたインターフェースになっています。RStanのホームページは以下になります。
...
3つ目のチューニングパラメータは仮想粒子の質量行列です。Stanはwarmupの間に対角行列となる質量行列や完全な質量行列を推定するように設定できます。また、将来的にはユーザが指定した質量行列をサポートする予定です。対角行列となる質量行列を推定すると、未知パラメータ系列$\theta$の各要素$\theta_k$のスケールは正規化されます。一方、完全な質量行列を推定すると、スケーリングと回転の両方を考慮することができますが、^[これらの推定された質量行列は大域的です。サンプリングされるパラメータ空間の全ての点に同じ質量行列が適用されるという意味です。これは、Riemann-manifold HMCではこれが一般化され、質量行列によって示唆される曲率が位置ごとに変化することが許されます。]行列演算のためにleapfrogの各ステップにおいて、より多くのメモリと計算を要します。

...
対角行列となる質量行列を推定すると，未知のパラメータの列 $\mathbb{theta}$ の成分 $\theta_k$ のスケールは正規化されます．一方完全な質量行列を推定すると，スケーリングと回転の両方を考慮する事が出来ますが，それと引き換えに，leapfrogの各ステップにおいてより多くのメモリを必要とします．

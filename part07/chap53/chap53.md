...
この章では、最尤推定と罰則付き最尤推定というベイズ推定ではない方法を定義し、事後分布の平均値・中央値・最頻値に基づいて、それらをベイズ点推定に関連づけます。事後分布ではなく、モデルのパラメーター$\theta$についての単一の値から求められるので、そのようにして推定された値は「点推定値」と呼ばれます。

どのような尤度関数も罰則関数もStanのモデリング言語でコーディングすることができるので、Stanのオプティマイザ（**optimizerの訳**）を使って（罰則付き）最尤推定を実装することができます。Stanのオプティマイザはまた、事後最頻値に基づくベイズの枠組み（**setting**）での点推定にも使うことができます。Stanのマルコフ連鎖モンテカルロサンプラーは、事後分布の平均値や中央値に基づくベイズモデルでの点推定を実装するのにも使えます。
...
- 2つ以上の点で尤度関数が最大になるとき
- 尤度関数が発散するとき
- 尤度関数が漸近的に収束して、有効な（**legalの訳**）パラメータの値に達しないとき
...
![$$(\hat{\beta},\hat{\sigma})=\mathrm{argmax}_{\beta,\sigma}\log p(y\mid\beta,\sigma,x) = \sum_{n=1}^{N} \log \mathsf{Normal}(y_n \mid x_n \beta,\sigma)$$](fig/fig04.png)


##### 二乗誤差

対数尤度関数について少し代数計算をすると、周辺最尤推定値$\hat{\theta}=(\hat{\beta},\hat{\sigma})$が、最小二乗法の$\beta$と等価に定式化できることが分かります。すなわち、$\hat{\beta}$は、二乗予測誤差の和を最小化するような係数ベクトルの値です。
...
線形回帰の二乗誤差法はStanでは以下のモデルで直接コーディングできます。
...
このモデルをStanのオプティマイザで走らせると、二乗誤差の和を直接最小化するとともに、ノイズのスケールを生成量として定義するのにその値を使って、線形回帰のMLEが生成されます。
...
最大化のとき、罰則関数$r(\theta)$は相殺されて（**negateの訳**）、推定値$\hat{\theta}$が、対数尤度の最大化と罰則の最小化との間でつりあいをとるようになります。罰則をつけることは「正則化」とも呼ばれます。

#### 例
##### リッジ回帰

リッジ回帰(Hoerl and Kennard, 1970)の基礎は、係数ベクトル$\beta$のユークリッド距離に罰則をつけるところにあります。次式がリッジ罰則関数です。
...
リッジ罰則関数を加える故知による効果は、$\beta$についてのリッジ回帰の推定値がより短いベクトルとなることです。すなわち$\hat{\beta}$が縮小されます。リッジ推定値は必ずしもすべての$k$について$\beta_k$の絶対値が小さくなるとは限りませんし、係数ベクトルが、最尤推定値と同じ方向を指すとも限りません。

Stanでリッジ罰則を加えるには、その大きさをデータ変数として加え、罰則自身を`model`ブロックに加えます。
...
Lassoは、L1ノルムとの関連から、L1縮小とも呼ばれます。L1ノルムは、タクシー距離あるいはマンハッタン距離とも言います。

罰則の導関数は$\beta_k$の値に依存しません。

![$$\frac{d}{d\beta_{k}}\lambda\sum_{k=1}^{K}|\beta_{k}|=\mathrm{signum}(\beta_{k})$$](fig/fig12.png)

そのため、縮小するパラメータは、最尤推定値では完全に0になるという効果があります。したがって、単に縮小だけではなく変数選択として使うこともできます。<sup>2</sup> Lassoも、大きさをデータとして宣言し、罰則を`model`ブロックに加えることにより、リッジ回帰と同じくらい簡単にStanで実装できます。
...
Elastic Net (Zou and Hastie, 2005)は、ナイーブElastic Netで生成された当てはめ値$\beta$から、最終的な$\beta$の推定値を調整するようにしています。Elastic Netの推定値は次式です。
...
観測値$y$が与えられたときのパラメータ$\theta$の事後分布$p(\theta \mid y)$に基づいてベイズ点推定を行なうのに普通に使うやり方は3つあります。すなわち、最頻値（最大値）、平均値、中央値です。この節では、事後分布を最大化するようなパラメータ$\theta$を基礎とする推定値について述べ、続いて次の節では平均値と中央値を議論します。

モデルの事後最頻値を基礎とする推定値は次式のように定義できます。
...
20章と53.1節で議論したように、ただひとつの事後最頻値は存在するとは限りません。事後最頻値を最大にするような値はひとつも存在しないこともありえますし、2つ以上のこともありえます。そのような場合、事後最頻値の推定値は定義されません。ほとんどのオプティマイザと同様に、Stanのオプティマイザでもそうした状況では問題が発生します。大域的には最大ではないような、局所最大値を返すこともありえます。

事後最頻値が存在する場合には、その値は、対数事前分布の反対に等しいような罰則関数を持つ罰則付き最尤推定値に対応するでしょう。これはベイズの定理から導かれます。(**多少意訳**)
...
普通に使われる罰則関数ほとんどについて、確率的に同じものが存在します。例えば、リッジ罰則関数は係数への正規事前分布に対応しますし、Lassoはラプラス事前分布に対応します。この逆も常に真です。事前分布の符号を負にしたもの(**nagative priorの訳**)は常に罰則関数と見なすことができます。
...
事後平均値が存在しないのに、事後最頻値は存在するという状況のひとつは、事後分布がコーシー分布$\mathsf{Cauchy}(\mu,\tau)$の場合です。事後最頻値は$\mu$ですが、事後平均値を表す積分は発散します。そのような幅の広い事前分布(**posteriorの誤り?**)は、実際にモデリングを使うときにはめったに出てきません。パラメータにコーシー分布の事前分布を使うときでも、データより十分な制約が与えられるので、事後分布は行儀が良くなり、平均値も存在するようになります。

事後平均値が存在しても、意味がないものであることもあります。混合分布モデルで起きる多峰の事後分布の場合や、閉区間での一様分布の場合がそれに当たります。

### 53.5. 事後中央値の推定

事後中央値（すなわち50番目の100分位点または0.5分位）は、ベイズモデルの報告によく使われる、もうひとつの点推定値です。事後中央値は、推定値の誤差の期待絶対値を最小化します。こうした推定値は、さまざまなStanのインターフェイスで返されます。フォーマットについてのさらに情報を得るにはRStan、PyStan、CmdStanのユーザーズガイドを参照してください。

事後中央値が意味のないものになることもありえますが、事後平均値が存在しないようなときでも多くの場合、事後中央値は存在します。コーシー分布もそのような場合です。

### 53.6 推定値の誤差、偏り、分散

推定値$\hat{\theta}$は、特定のデータ$y$のほか、対数尤度関数$\log p(y \mid \theta)$、罰則付き尤度関数$\log p(y \mid \theta) - r(\theta)$、対数確率関数$\log p(y,\theta) = \log p(y \mid \theta) + log p(\theta)$（**原文は$\log p(y,\theta) = \log p(y, \theta) + log p(\theta)$だがこれは誤りであろう**）のうちのいずれかに依存します。この節では、$\hat{\theta}$という記法は推定量を示すものとしても定義します。このときの推定量とは、データと（罰則付き）尤度あるいは確率関数の非明示的な関数です。
...
特定の真のパラメータの値$\theta$と尤度関数$p(y \mid \theta)$について、その尤度のもとでの密度に従うようなデータセット$y$の取りうる範囲全体について平均した推定誤差の期待値（**ここはわかりにくい**）は次式のとおりです。
...
偏りは$\theta$と同じ次元の多変量です。この期待推定誤差が0であれば推定量は不偏ですが、そうでなければ偏りがあります。

##### 例: 正規分布の推定値

正規分布から抽出された、$n \in 1:N$についての観測値$y_n$からなるデータセットがあるとします。これは、$y_n \sim \mathsf{Normal}(\mu, \sigma)$というモデルを仮定しています。ここで、$\mu$と$\sigma$はともに$>0$であるようなパラメータです。尤度は次式のとおりです。
...
平均値の推定値$\hat{\mu}$との差に最尤推定値が基づいていることがこの偏りの理由です。実際の平均値をいれてみると、差の二乗和が大きくなります。すなわち、$\mu \neq \hat{\mu}$ならば、次式のようになります。

分散の推定値はもうひとつあり、それが標本分散です。次式のように定義されます。

![$$\frac{1}{N}\sum_{n=1}^{N}(y_{n}-\mu)^2 > \frac{1}{N}\sum_{n=1}^{N}(y_{n}-\hat{\mu})^2$$](fig/fig27.png)

この値は、最尤推定値よりも$N/(N-1)$倍だけ大きくなります。

![$$\hat{\mu} = \frac{1}{N-1}\sum_{n=1}^{N}(y_{n}-\hat{\mu})^2$$](fig/fig28.png)
...
標本データから正規分布の平均と分散を推定する例での計算では、最尤推定量（すなわち標本平均）は、分散を最小とするような平均$\mu$の不偏推定量です。ガウス-マルコフの定理が、最小二乗推定について一定の（**some**）一般性をもって、すなわち、正規ノイズの過程のもとでの最尤推定値と等価であるとして、この結果を証明しました。（**この文は要見直し**）
...
この時，`dim(x)`や`dim(y)`を呼ぶことで，含まれる要素が順に7, 8, 9となるサイズ3の整数配列が返されます。
...
`v`のインデックスを昇順に並べ替えた，1から`v`のサイズまでのインデックスが入った配列を返します。
...
`v`のインデックスを降順に並べ替えた，1から`v`のサイズまでのインデックスが入った配列を返します。

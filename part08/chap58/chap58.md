...
実際サンプリングの効率、1回のサンプリングとその繰り返しの速度はこの3つのパラメータに大きく依存します(Neal, 2011;Hoffman and Gelman, 2014)。
...
質量行列Σが事後分布の共分散に合致していなければ同じ時間に対して計算の正確性を確保するためのステップサイズεは小さくなり、統計的な効率性を保つだけの時間に必要なステップ数Lは大きくなってしまいます。　
...
これらの期間は以下のような調整パラメータに乗っ取って制御されます。各パラメータは正の整数である必要があります。
...
このwarmupの手続きはとても柔軟で完全なもので、Stanでは
　Hoff- man and Gelman (2014)の記法で外部に出しています。
実際最適化の有効性はこのパラメータの値に対して敏感に変化します。しかし我々は双平均化法を使った経験あるのではない限りパラメータの値をデフォルト値から動かすことは推奨しません。
...
最初の数回のイテレーションの受理確率とstep sizeを見るとこれがいかに悪い問題でモデリングの努力
